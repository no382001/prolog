...
@test "unconsult reclaims perm pool" {
  ...
  after_out=$("$TRILOG" -s -e "consult('ledit.pl'), unconsult('ledit.pl')." 2>&1 1>/dev/null)
...
@test "double consult/unconsult cycle: no growth" {
  once=$("$TRILOG" -s -e "consult('ledit.pl'), unconsult('ledit.pl')." 2>&1 1>/dev/null)
  p1=$(get_stat perm_pool "$once")

  twice=$("$TRILOG" -s -e "consult('ledit.pl'), unconsult('ledit.pl'), consult('ledit.pl'), unconsult('ledit.pl')." 2>&1 1>/dev/null)
...
@test "core predicates survive consult/unconsult" {
  run "$TRILOG" -e "consult('ledit.pl'), unconsult('ledit.pl'), append([1,2],[3,4],X), write(X)."
  [ "$status" -eq 0 ]
  [[ "$output" == *"[1, 2, 3, 4]"* ]]
}

@test "core predicates survive double cycle" {
  run "$TRILOG" -e "consult('ledit.pl'), unconsult('ledit.pl'), consult('ledit.pl'), unconsult('ledit.pl'), findall(X, member(X,[a,b,c]), L), write(L)."
  [ "$status" -eq 0 ]
  [[ "$output" == *"abc"* ]]
}

@test "assert/retract cycle after unconsult" {
  run "$TRILOG" -e "consult('ledit.pl'), unconsult('ledit.pl'), assert(tmp(1)), assert(tmp(2)), retract(tmp(1)), tmp(X), write(X)."
  [ "$status" -eq 0 ]
  [[ "$output" == *"2"* ]]
}
